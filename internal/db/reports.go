...
func GetReportByUserID(userID string) (string, string) {
	rows, err := DBClient.Query("SELECT channel_id, channel_name FROM reports WHERE user_id = ? AND status = ?", userID, "open")
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func GetReportByChannelID(channelID string) (string, string) {
	rows, err := DBClient.Query("SELECT user_id, channel_name FROM reports WHERE channel_id = ? AND status = ?", channelID, "open")
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func checkUniqueOpenChannelName(channelName string) bool {
	rows, err := DBClient.Query("SELECT channel_name FROM reports WHERE channel_name = ? AND status = ?", channelName, "open")
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func CreateReport(channelID, channelName, userID string) {
	...
	_, err := DBClient.Exec(`INSERT INTO reports (channel_id, channel_name, user_id, status, created_at, updated_at)
...
func CloseReport(channelID string) {
	_, err := DBClient.Exec(`UPDATE reports SET status = ?, updated_at = ? WHERE channel_id = ?`, "closed", "now", channelID)
	if err != nil {
		panic(err)
	}
}
